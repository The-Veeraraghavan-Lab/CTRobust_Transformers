...
def compute_meandice(y_pred: torch.Tensor, y: torch.Tensor, include_background: bool = True) -> torch.Tensor:
    ...
    #print (' y is ',y.shape) # y_pred is  torch.Size([1, 16, 128, 128, 128])
    #print (' y is ',y)
    #print (' y_pred is ',y_pred.shape) # y_pred is  torch.Size([1, 16, 128, 128, 128])
    #print (' y_pred is ',y_pred)
    intersection = torch.sum(y * y_pred, dim=reduce_axis)

    y_o = torch.sum(y, reduce_axis)
    y_pred_o = torch.sum(y_pred, dim=reduce_axis)
    denominator = y_o + y_pred_o +0.0000001

    #return torch.where(y_o > 0, (2.0 * intersection) / denominator, torch.tensor(float("nan"), device=y_o.device))
    #Jue fix bug for denominator is 0
